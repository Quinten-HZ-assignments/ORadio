...
class RadioStateMachine(StateMachine):
    ...
    def on_enter_off(self):
        ...
        global data
        data.append(Interaction(datetime.now(), 'Radio Off', Switch_sel1_count + 1))
        parseToCsv(data)
        data = []
        #        print('stop; off')
        led_green.off()
    ...
    def on_enter_playPlayL1(self):
        ...
        global data
        data.append(Interaction(datetime.now(), 'Sixties', Switch_sel1_count + 1))
        parseToCsv(data)
        data = []
        led_green.on()
        #        vib_motor.blink(on_time=0.2,off_time=0.2,n=2,background=True)  #vib_motor 0.2 s on, 1 s off repeat 3, in background)
    ...
    def on_enter_playPlayL2(self):
        global PlayListStopFlag, PlayListName, offlineFlag
        global data
        data.append(Interaction(datetime.now(), 'Nostalgia', Switch_sel1_count + 1))
        parseToCsv(data)
        data = []
        #	print('play L2')
        led_green.on()
    ...
    def on_enter_playPlayL3(self):
        global PlayListStopFlag, PlayListName, offlineFlag
        global data
        data.append(Interaction(datetime.now(), 'Klassiek', Switch_sel1_count + 1))
        parseToCsv(data)
        data = []
        #        print('play L3')
        led_green.on()
    ...
    def on_enter_playPlayL4(self):
        global PlayListStopFlag, PlayListName, offlineFlag
        global data
        data.append(Interaction(datetime.now(), 'Nederlands', Switch_sel1_count + 1))
        parseToCsv(data)
        data = []
        #        print('play L4')
        led_green.on()
...
def switchOn():
    global data
    data.append(Interaction(datetime.now(), 'Radio on', 1))
    parseToCsv(data)
    data = []
    print("-----------------------------Switch on-------------------------------",
...
def Volume_read():
    ...
    if abs(VolumeSetRaw_prev - VolumeSetRaw_last) > 2:
        ...
        #               client.disconnect()
        # -----
        # data.append(Interaction(datetime.now(), 'Volume', VolumeSet))
        # parseToCsv(data)
        # data = []

...
def parseToCsv(data):
    ...
    if not path.exists('buttonslog.csv'):
        with open('buttonslog.csv', 'a') as f:
            writer = csv.writer(f)
            writer.writerow(['Time', 'Button', 'device_state', 'volume_dial_position'])

    try:
        filename = 'buttonslog.csv'
        with open(filename, 'a') as f:
            writer = csv.writer(f)
            writer.writerow([data.id, data.name, data.volume_dial, data.state])
...
def Volume_read():
    global VolumeSetRaw_prev, VolumeSetRaw, data
    ...
    if abs(VolumeSetRaw_prev - VolumeSetRaw_last) > 2:
        ...
        #               client.disconnect()
        # -----
        parseToCsv(Interaction(datetime.now(),'Volume',device_state))

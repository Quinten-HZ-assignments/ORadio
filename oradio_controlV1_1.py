...
def switchOn():
    ...
    if Switch_sel1.value > 0:
        data.append(Interaction(datetime.now(), 'Switch_sel1', Switch_sel1_count + 1))
        Radio_state.playL1()
    if Switch_sel2.value > 0:
        data.append(Interaction(datetime.now(), 'Switch_sel2', Switch_sel2_count + 1))
        Radio_state.playL2()
    if Switch_sel3.value > 0:
        data.append(Interaction(datetime.now(), 'Switch_sel3', Switch_sel3_count + 1))
        Radio_state.playL3()
    if Switch_sel4.value > 0:
        data.append(Interaction(datetime.now(), 'Switch_sel4', Switch_sel4_count + 1))
        Radio_state.playL4()


# ----------Switch off with switch-------------------------
def switchOff():
    data.append(Interaction(datetime.now(), 'Switch_off', Switch_off_count + 1))

    df = pd.DataFrame(data)
    df.to_csv('buttonLogging.csv')
